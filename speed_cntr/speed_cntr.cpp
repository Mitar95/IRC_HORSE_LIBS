...
void speed_cntr_Move(signed int step, unsigned int accel, unsigned int decel, unsigned int speed)
{
  ...
  // Only move if number of steps to move is not zero.
    ...
    srd.step_delay = (T1_FREQ_148 * sqrt_NEW(A_SQ / accel))/100;
...
void speed_cntr_Init_Timer1(void)
{
  ...
  cli(); // stop interrupts
	...
	OCR1A = 100; // = 16000000 / (8 * 100) - 1 (must be <65536)
				   // turn on CTC mode
	TCCR1B |= (1 << WGM12);
 ...
 */
 unsigned long sqrt_NEW(unsigned long x) // static
{
 ...
 *  \return  Min(x,y).
 */
unsigned int min_NEW(unsigned int x, unsigned int y)
{
  if(x < y){
    return x;
  }
...
int AngleToStep(float angle){
  return (int)((6400.0 / 360.0) * angle);
}

void Calibrate(uint8_t calibratePin){
  while(digitalRead(calibratePin) == LOW){ // HIGH
